...
export function ArticleLayout({ article }: { article: Article }) {
  ...
  const related = ARTICLES
    .filter((candidate) => candidate.slug !== article.slug && candidate.keywords.some((keyword) => article.keywords.includes(keyword)))
    .slice(0, 3);
      ...
      <article className="mx-auto max-w-3xl px-5 sm:px-6 pb-12">
        <header className="mb-8">
          <div className="inline-flex items-center gap-2 text-[10px] font-mono uppercase tracking-widest text-accent mb-4">
            ...
            <span className="inline-flex items-center gap-1 text-muted-foreground">
              ...
              {new Date(article.publishedISO).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" })}
            </span>
            <span className="inline-flex items-center gap-1 text-muted-foreground">
        ...
        <div className="mt-12 border-t border-border pt-8 flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <div className="text-sm mt-1">Share with someone who would benefit.</div>
          </div>
          ...
          <section className="mt-12 border-t border-border pt-10">
            ...
            <div className="grid sm:grid-cols-3 gap-4">
              {related.map((relatedArticle) => (
                <Link key={relatedArticle.slug} to={`/insights/${relatedArticle.slug}`} className="group block border border-border rounded-xl p-4 hover:border-accent/50 hover:bg-secondary/40 transition">
                  <div className="text-[10px] font-mono uppercase tracking-widest text-accent mb-1.5">{relatedArticle.category}</div>
                  <h3 className="text-sm font-semibold leading-snug group-hover:text-accent transition-colors">{relatedArticle.title}</h3>
                  <div className="mt-3 inline-flex items-center gap-1 text-[11px] text-muted-foreground">
                    <Clock className="h-3 w-3" />
                    {relatedArticle.readMinutes} min
                  </div>
                </Link>
              ))}
            </div>
